...
void BayesianMatting::CollectFgSamples(const int x, const int y, std::vector<std::pair<cv::Vec3f, float> > *sample_set)
{
    ...
    while (sample_set->size() < nearest)
    {
        if (y - dist >= 0)
        {
            ...
            {
                dist_weight = std::exp(-(dist * dist + (z - x) * (z - x)) * inv_2sigma_square);
        ...
        if (x - dist >= 0)
        {
            ...
            {
                dist_weight = std::exp(-(dist * dist + (z - y) * (z - y)) * inv_2sigma_square);
        ...
        if (x + dist < fgImg.cols)
        {
            for (int z = std::max(0, y - dist + 1); z <= std::min(fgImg.rows - 1, y + dist - 1); z++)
            {
                dist_weight = std::exp(-(dist * dist + (y - z) * (y - z)) * inv_2sigma_square);
...
void BayesianMatting::Solve(void)
{
	...
	//std::cout << "===================================================================" << std::endl;
	std::cout << "There are total " << nUnknown << " pixels need to be solved" << std::endl;
